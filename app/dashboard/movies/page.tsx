...
export default async function MoviesPage() {
  ...
  const movies = await db.movie.findMany({
    where: { userId },
    select: {
      title: true, plays: true, watchedAt: true,
  ...
  const totalHours = Math.round(movies.reduce((s, m) => s + (m.runtime ?? 0) * m.plays, 0) / 60)
  ...
  const top10 = [...movies].sort((a, b) => b.plays - a.plays).slice(0, 10)
  const highestRated = [...movies].filter((m) => m.rating).sort((a, b) => (b.rating ?? 0) - (a.rating ?? 0)).slice(0, 20)
  const genres = groupByGenre(movies)
  const releasedYear = groupByReleasedYear(movies)
  const ratings = ratingDistribution(movies)
  ...
  return (
    <div className="flex flex-col gap-10">
      ...
      <section className="rounded-xl border border-zinc-800 bg-zinc-900 p-6">
        ...
        <PlaysByTimeCharts data={charts} type="movies" />
      </section>

      <section>
        <h2 className="text-lg font-semibold mb-4">Top 10 Movies</h2>
          ...
          {top10.map((m) => (
            <div key={m.title} className="flex flex-col gap-1">
              <div className="relative aspect-[2/3] rounded overflow-hidden bg-zinc-800">
                {m.poster && (
                  <Image src={`https://image.tmdb.org/t/p/w200${m.poster}`} alt={m.title} fill className="object-cover" />
                )}
              </div>
              <span className="text-xs text-zinc-400 leading-tight">{m.title}</span>
              <span className="text-xs text-zinc-600">{m.plays} {m.plays === 1 ? "play" : "plays"}</span>
            </div>
          ))}
        </div>
      </section>
      ...
      {highestRated.length > 0 && (
        <section>
          ...
          <div className="flex gap-2 overflow-x-auto pb-2">
            {highestRated.map((m) => (
              <div key={m.title} className="relative shrink-0 w-24">
                <div className="relative aspect-[2/3] rounded overflow-hidden bg-zinc-800">
                  {m.poster && (
                    <Image src={`https://image.tmdb.org/t/p/w200${m.poster}`} alt={m.title} fill className="object-cover" />
                  )}
